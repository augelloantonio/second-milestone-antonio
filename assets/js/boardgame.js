...
function startGame() {
    ...
    //delete//
    $.getScript("/assets/js/timer.js", function( data, textStatus, jqxhr ) {
});
    document.getElementById("startBtn").onclick = function() { newBoard() };
    document.getElementById("boardgame").style.display = "inline-block";
}
...
function levelTwoBoard() {
    $.getScript("/assets/js/levels/levelTwo.js", function( data, textStatus, jqxhr ) {
});
}
        ...
        if (memory_values.length == 0) {
            ...
            if (memory_values[0] == memory_values[1]) {
                ...
                if (card_flipped == memory_array.length) {
                    alert("Well done! But this one was an easy one, let's go ahead! Level Two Starting!");
                ...
                function flip2Back() {
                    ...
                    card_1.style.cssText = 'background: url(/assets/img/card.png) no repeat, background-size: cover';
                    card_1.innerHTML = "";
                    card_2.style.cssText = 'background: url(/assets/img/card.png) no repeat, background-size: cover';
                    card_2.innerHTML = "";
                    // Clear both arrays
                    memory_values = [];
                    memory_card_ids = [];
